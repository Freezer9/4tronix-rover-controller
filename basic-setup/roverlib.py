...
# python list of 48 positions to hold calibration data retrieved from EEROM
servoOffsets = [0]*48

# ======================================================================
# Fixed base I2C Address of the PCA9685 chip (for SERVO control)
PCAbaseAddress = 0x40
PCAbaseOffset = 6
...
def initMotors():
    ...
    LeftMotorsForward = GPIO.PWM(LeftMotorPin1, 1000)
    LeftMotorsForward.start(0)

    GPIO.setup(LeftMotorPin2, GPIO.OUT)
    LeftMotorsReverse = GPIO.PWM(LeftMotorPin2, 1000)
    LeftMotorsReverse.start(0)

    GPIO.setup(RightMotorPin1, GPIO.OUT)
    RightMotorsForward = GPIO.PWM(RightMotorPin1, 1000)
    RightMotorsForward.start(0)

    GPIO.setup(RightMotorPin2, GPIO.OUT)
    RightMotorsReverse = GPIO.PWM(RightMotorPin2, 1000)
...
#     # implements requested mode except if obstacle detected or power = 0
...
def setServo(Servo, Degrees):
    ...
    if Degrees >= -90 and Degrees <= 90:
        ...
        # offset relative to standard PWMvalue 375 for 0' mid position
        midOffset = servoOffsets[Servo]
        # offset relative to standard PWMvalue 575 for 90' right position
        rightOffset = servoOffsets[Servo+16]
        # offset relative to standard PWMvalue 175 for 90' left position
        leftOffset = servoOffsets[Servo+32]

        # calculate PWM value for either right or left quadrant movement
        if Degrees >= 0:
            PWMvalue = int((375+midOffset) + (Degrees/90 *
                           (575+rightOffset-375-midOffset)))
        else:
            PWMvalue = int((375+midOffset) + (Degrees/90 *
                           (375+midOffset-175-leftOffset)))
...
def setWheelServosSmooth(FLdegrees, FRdegrees, RLdegrees, RRdegrees):
    ...
    maxDiff = max(math.fabs(FLdiff), math.fabs(FRdiff),
                  math.fabs(RLdiff), math.fabs(RRdiff))
    if maxDiff > 2:
...
def initI2C():
    ...
    try:
        ...
        bus.write_byte_data(PCAbaseAddress, 1, 0x04)
        # get current Mode1 register
        mode1 = bus.read_byte_data(PCAbaseAddress, 0)
...
def writePWM(channel, PWMvalue):
    ...
    if I2C_INITIALIZED:
        if (channel >= 0) and (channel <= 16):
            ...
            stop = PWMvalue
            bus.write_byte_data(
                PCAbaseAddress, PCAbaseOffset + channel*4 + 0, start & 0xff)
            bus.write_byte_data(
                PCAbaseAddress, PCAbaseOffset + channel*4 + 1, start >> 8)
            bus.write_byte_data(
                PCAbaseAddress, PCAbaseOffset + channel*4 + 2, stop & 0xff)
            bus.write_byte_data(
                PCAbaseAddress, PCAbaseOffset + channel*4 + 3, stop >> 8)
    else:
...
def stopPWM(channel):
    if I2C_INITIALIZED:
        if (channel >= 0) and (channel <= 16):
            bus.write_byte_data(
                PCAbaseAddress, PCAbaseOffset + channel*4 + 0, 0)
            bus.write_byte_data(
                PCAbaseAddress, PCAbaseOffset + channel*4 + 1, 0)
            bus.write_byte_data(
                PCAbaseAddress, PCAbaseOffset + channel*4 + 2, 0)
            bus.write_byte_data(
                PCAbaseAddress, PCAbaseOffset + channel*4 + 3, 0)
    else:
        print("Error: I2C for PWM channel not initialized")
# ======================================================================
# end of PCA9685
...
def wrEEROM(Address, Data):
    bus.write_i2c_block_data(EEROMbaseAddress, Address >> 8, [
                             Address & 0xff, Data])
    time.sleep(0.01)
...
def matchLedsToDrive(driveType, direction, powerpct, radiusCm):
    ...
    if powerpct != 0:
        if radiusCm > 0 and radiusCm < 200:
            ...
            # without blinking 2 threads are active: main and obstacle detection
            alreadyBlinking = not (threading.activeCount() == 2)
...
def blinkLedNoWait(LedID, color, blinkPerSecond, direction):
    ...
    global stopBlink
    stopBlink = False
    thread = threading.Thread(target=blinkLed, args=(
        LedID, color, blinkPerSecond, direction))
    thread.start()
#
...
def detectObstacle():
    ...
    while not stopSonar:
        ...
        print(f"[SONAR] distance: {sonarDistance:.1f} cm")
        # treat 0 as sensor error, not a real obstacle
        if sonarDistance > 0 and sonarDistance <= 20:
...
def Ackermandrive(driveType, powerpercent, turningRadiusCm):
    ...
    # angle from center of driving circle (depending on turningRadiusM) to center of wheel
    FLangle = math.atan(distFLys/(math.fabs(turningRadiusM)+distFLxs))
    # width center of wheel to center of servo, depending on angle
    distFLxws = math.cos(FLangle)*distFLws
    # length center of wheel to center of servo, depending on angle
    distFLyws = math.sin(FLangle)*distFLws
    # turningRadiusM from center of driving circle to center of wheel
    FLturningRadiusM = math.hypot(
        (math.fabs(turningRadiusM)+distFLxs+distFLxws), (distFLys+distFLyws))
    # wheel angle in degrees (turn wheel right)
    FLdegrees = math.degrees(FLangle)
    # set Front Left powerpercent to requested powerpercent
    FLpowerpercent = powerpercent

    # turningRadiusM from center of driving circle to center of wheel
    MLturningRadiusM = math.fabs(turningRadiusM)+distMLxs
    # set Middle Left powerpercent relative to Front Left powerpercent
    MLpowerpercent = MLturningRadiusM/FLturningRadiusM*powerpercent

    # angle from center of driving circle (depending on turningRadiusM) to center of wheel
    RLangle = math.atan(distRLys/(math.fabs(turningRadiusM)+distRLxs))
    # width center of wheel to center of servo, depending on angle
    distRLxws = math.cos(RLangle)*distRLws
    # length center of wheel to center of servo, depending on angle
    distRLyws = math.sin(RLangle)*distRLws
    # turningRadiusM from center of driving circle to center of wheel
    RLturningRadiusM = math.hypot(
        (math.fabs(turningRadiusM)+distRLxs+distRLxws), (distRLys+distRLyws))
    # wheel angle in degrees (turn wheel left) note the minus sign !
    RLdegrees = -1*math.degrees(RLangle)
    # set rear Left powerpercent relative to Front Left powerpercent
    RLpowerpercent = RLturningRadiusM/FLturningRadiusM*powerpercent

    # note minus signs in below formulas !

    # angle from center of driving circle (depending on turningRadiusM) to center of wheel
    FRangle = math.atan(distFRys/(math.fabs(turningRadiusM)-distFRxs))
    # width center of wheel to center of servo, depending on angle
    distFRxws = math.cos(FRangle)*distFRws
    # length center of wheel to center of servo, depending on angle
    distFRyws = math.sin(FRangle)*distFRws
    # turningRadiusM from center of driving circle to center of wheel
    FRturningRadiusM = math.hypot(
        (math.fabs(turningRadiusM)-distFRxs-distFRxws), (distFRys-distFRyws))
    # wheel angle in degrees (turn wheel right)
    FRdegrees = math.degrees(FRangle)
    # set Front Right powerpercent relative to Front Left powerpercent
    FRpowerpercent = FRturningRadiusM/FLturningRadiusM*powerpercent

    # turningRadiusM from center of driving circle to center of wheel
    MRturningRadiusM = math.fabs(turningRadiusM)-distMRxs
    # set Middle Right powerpercent relative to Front Left powerpercent
    MRpowerpercent = MRturningRadiusM/FLturningRadiusM*powerpercent

    # angle from center of driving circle (depending on turningRadiusM) to center of wheel
    RRangle = math.atan(distRRys/(math.fabs(turningRadiusM)-distRRxs))
    # width center of wheel to center of servo, depending on angle
    distRRxws = math.cos(RRangle)*distRRws
    # length center of wheel to center of servo, depending on angle
    distRRyws = math.sin(RRangle)*distRRws
    # turningRadiusM from center of driving circle to center of wheel
    RRturningRadiusM = math.hypot(
        (math.fabs(turningRadiusM)-distRRxs-distRRxws), (distRRys-distRRyws))
    # wheel angle in degrees (turn wheel left) note the minus sign !
    RRdegrees = -1*math.degrees(RRangle)
    # set rear Right powerpercent relative to Front Left powerpercent
    RRpowerpercent = RRturningRadiusM/FLturningRadiusM*powerpercent

    # switch all wheel settings left/right around if turningRadiusM is negative (turn left)
    if (turningRadiusM < 0):
    ...
    # spin around center, make left wheels turn in opposite direction
    if (turningRadiusM == 0):
